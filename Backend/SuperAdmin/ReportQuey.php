...
function BestSellerByTime($StartDate, $EndDate)
{
    global $connectDB;
    if ($StartDate == $EndDate) {
        if ($StartDate == $EndDate) {
    ...
    $stmt = $connectDB->prepare("SELECT p.ProID, p.ProName, SUM(rl.Qty) as Qty FROM RECEIPT_LIST rl JOIN PRODUCT p ON rl.ProID = p.ProID JOIN RECEIPT r ON rl.RecID = r.RecID WHERE r.PayTime >= ? AND r.PayTime <= ? GROUP BY rl.ProID ORDER BY Qty DESC LIMIT 5");
    $stmt->bind_param("ss", $StartDate, $EndDate);
    $stmt->execute();
    $result = $stmt->get_result();
    return $result;
